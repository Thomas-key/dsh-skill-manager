...
export function apply(ctx) {
  ...
  // ── same-origin JSON routes for the bundle client half (Settings UI) ──
  // The bundle client (lib/client.js) cannot use harness.handle/host.call
  // (those are dynamic-plugin only); it fetches these routes instead. The
  // fence is copied from the skin-center pattern: same-origin only.
    ...
    const routes = [
      {
        ...
        handler: async (req, res) => {
          ...
          try {
            const list = await runBridge({ op: 'list' }, undefined)
            json(res, 200, { ok: true, skills: Array.isArray(list) ? list : [] })
          } catch (error) {
      ...
      },
      {
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            return json(res, 400, { ok: false, error: 'unreadable body' })
          }
          let args = {}
          try {
            args = JSON.parse(raw || '{}')
          ...
          try {
            const result = await runBridge({ op: 'toggle', name, enabled: args.enabled === true }, undefined)
